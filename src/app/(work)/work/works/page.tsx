...
const WorkModal = dynamic(() =>
  import("@/app/components/work-modal/work-modal").then((m) => m.WorkModal)
);
...
export default function Work() {
  ...
  useEffect(() => {
    if (view) {
      router.replace(
        `/work/works?${createQueryString(viewSearchParamKey, view)}`
      );
    }
    ...
    <section>
      ...
      <div className="flex flex-wrap justify-between mb-8 gap-4">
        ...
        <div className="flex justify-center gap-4 mr-2">
          ...
          <Link
            href="/cv"
            className={`cursor-pointer bg-white grid place-items-center gap-0.5 h-[26px] w-[26px] opacity-25 md:hover:opacity-100`}
          >
            <span className="text-sm text-black font-semibold">CV</span>
          </Link>
        </div>
      </div>
      <AnimatePresence>
